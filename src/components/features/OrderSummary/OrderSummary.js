...
import ChangeAmount from '../../common/ChangeAmount/ChangeAmount';
...
const OrderSummary = ({totalPrice, products, editable}) => {
  ...
  const renderAmount = (amount) => (
    <ChangeAmount value={amount} isReadOnly={!editable} min={0} max={9} />
  );
    ...
    <Paper variant='outlined' className={styles.component}>
      <Grid container spacing={2} direction='column'>
        ...
        <Grid item className={styles.table}>
          <DataGrid rows={products} columns={columns} autoHeight disableColumnSelector disableSelectionOnClick disableColumnMenu hideFooter headerHeight={36} rowHeight={150} />
        </Grid>
        <Grid item>
